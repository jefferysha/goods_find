# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## 项目概述

基于 Playwright + AI 的二手商品智能监控分析平台。FastAPI 后端 + React 前端，支持多任务并发监控、多模态 AI 商品分析、溢价分析、智能提醒、多渠道通知推送。

## 核心架构

```
API层 (src/api/routes/)
    ↓
服务层 (src/services/)
    ↓
领域层 (src/domain/)
    ↓
基础设施层 (src/infrastructure/)
```

关键入口：
- `src/app.py` - FastAPI 应用主入口
- `spider_v2.py` - 爬虫 CLI 入口
- `src/scraper.py` - Playwright 爬虫核心逻辑

服务层：
- `TaskService` - 任务 CRUD
- `ProcessService` - 爬虫子进程管理
- `SchedulerService` - APScheduler 定时调度
- `AIAnalysisService` - 多模态 AI 分析
- `NotificationService` - 多渠道通知（ntfy/Bark/企业微信/Telegram/Webhook）
- `PricingService` - 溢价分析
- `HistoryService` - 历史价格追踪
- `AlertService` - 智能提醒规则引擎
- `DashboardService` - 数据仪表盘
- `FavoriteService` - 收藏与对比

前端 (`web-ui/`)：React 18 + TypeScript + Vite + shadcn/ui + Tailwind CSS + Three.js

## 开发命令

```bash
# 后端开发
uv run python -m src.app
# 或
uv run uvicorn src.app:app --host 0.0.0.0 --port 8000 --reload

# 前端开发
cd web-ui && pnpm install && pnpm run dev

# 前端构建
cd web-ui && pnpm run build

# 一键本地启动（构建前端 + 启动后端）
bash start.sh

# Docker 部署
docker compose up --build -d
```

## 依赖管理

```bash
# Python 依赖 (uv)
uv sync                    # 安装所有依赖
uv sync --no-dev           # 仅安装生产依赖
uv add <package>           # 添加依赖
uv add --dev <package>     # 添加开发依赖
uv lock                    # 更新锁文件

# 前端依赖 (pnpm)
cd web-ui && pnpm install
cd web-ui && pnpm add <package>
```

## 爬虫命令

```bash
uv run python spider_v2.py                          # 运行所有启用任务
uv run python spider_v2.py --task-name "MacBook"    # 运行指定任务
uv run python spider_v2.py --debug-limit 3          # 调试模式，限制商品数
uv run python spider_v2.py --config custom.json     # 自定义配置文件
```

## 测试

```bash
uv run pytest                              # 运行所有测试
uv run pytest --cov=src                    # 覆盖率报告
uv run pytest tests/unit/test_utils.py    # 运行单个测试文件
uv run pytest tests/unit/test_utils.py::test_safe_get  # 运行单个测试函数
```

测试规范：文件 `tests/**/test_*.py`，函数 `test_*`

## 配置

环境变量 (`.env`)：
- AI 模型：`OPENAI_API_KEY`, `OPENAI_BASE_URL`, `OPENAI_MODEL_NAME`
- 通知：`NTFY_TOPIC_URL`, `BARK_URL`, `WX_BOT_URL`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`
- 爬虫：`RUN_HEADLESS`, `LOGIN_IS_EDGE`
- Web 认证：`WEB_USERNAME`, `WEB_PASSWORD`
- 端口：`SERVER_PORT`

任务配置 (`config.json`)：定义监控任务（关键词、价格范围、cron 表达式、AI prompt 文件等）

## 数据流

1. Web UI / config.json 创建任务
2. SchedulerService 按 cron 触发或手动启动
3. ProcessService 启动 spider_v2.py 子进程
4. scraper.py 使用 Playwright 抓取商品
5. AIAnalysisService 调用多模态模型分析
6. PricingService 计算溢价率并分级
7. AlertService 匹配提醒规则
8. NotificationService 推送符合条件的商品
9. 结果存储：`jsonl/`（数据）、`images/`（图片）、`logs/`（日志）、`data/monitor.db`（SQLite）

## 注意事项

- AI 模型必须支持图片上传（多模态）
- Docker 部署需通过 Web UI 手动更新登录状态（`state.json`）
- 遇到滑动验证码时设置 `RUN_HEADLESS=false` 手动处理
- 生产环境务必修改默认 Web 认证密码
